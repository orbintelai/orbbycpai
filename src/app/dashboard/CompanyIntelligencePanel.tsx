...
function ModuleHeading({ module, status }: { module: IntelligenceModule; status?: ModuleStatus }) {
  ...
  const text = status?.status === "published" ? "First-party source" : status?.status === "blocked" ? "Source restricted access" : "Not published";
  return (
...
function EmptyModule({ status }: { status?: ModuleStatus }) {
  return <p style={{ margin: 0, color: "rgba(255,255,255,0.42)", fontSize: 12, lineHeight: 1.6 }}>{status?.reason || "This company has not published this information on its first-party website."}</p>;
}

const cardStyle: React.CSSProperties = {
  border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.025)", borderRadius: 12, padding: "16px 18px",
};
  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
        <div style={cardStyle}>
          <ModuleHeading module="people" status={statuses?.people} />
          {intel?.people?.length ? <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{intel.people.slice(0, 8).map((person) => <div key={`${person.name}-${person.title}`} style={{ display: "flex", gap: 9, alignItems: "flex-start" }}><div style={{ width: 25, height: 25, borderRadius: "50%", background: "rgba(80,227,194,0.14)", color: "#50e3c2", display: "grid", placeItems: "center", fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{person.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</div><div style={{ minWidth: 0 }}><div style={{ color: "rgba(255,255,255,0.82)", fontSize: 12, fontWeight: 600 }}>{person.name}<EvidenceLinks items={person.evidence} /></div><div style={{ color: "rgba(255,255,255,0.45)", fontSize: 11, marginTop: 2 }}>{person.title}</div></div></div>)}</div> : <EmptyModule status={statuses?.people} />}
        </div>

        <div style={cardStyle}>
          <ModuleHeading module="hiring" status={statuses?.hiring} />
          {intel?.hiring?.totalOpenRoles ? <><div style={{ fontSize: 30, lineHeight: 1, color: "#fff", fontWeight: 750 }}>{intel.hiring.totalOpenRoles}</div><div style={{ color: "rgba(255,255,255,0.42)", fontSize: 11, marginTop: 5, marginBottom: 13 }}>publicly listed open roles</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{intel.hiring.byDepartment.slice(0, 6).map((item) => <span key={item.name} style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.62)", borderRadius: 99, fontSize: 10, padding: "4px 8px" }}>{item.name} · {item.count}</span>)}</div>{intel.hiring.roles.some((role) => role.leadership) && <div style={{ color: "#f3b562", marginTop: 12, fontSize: 11 }}>Leadership hiring: {intel.hiring.roles.filter((role) => role.leadership).slice(0, 3).map((role) => role.title).join(", ")}</div>}</> : <EmptyModule status={statuses?.hiring} />}
        </div>

        <div style={cardStyle}>
          <ModuleHeading module="compliance" status={statuses?.compliance} />
          {intel?.compliance?.length ? <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>{intel.compliance.map((claim) => <div key={`${claim.framework}-${claim.claim}`}><div style={{ color: "#e6b7ff", fontSize: 11, fontWeight: 700 }}>{claim.framework}<EvidenceLinks items={claim.evidence} /></div><div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, lineHeight: 1.5, marginTop: 3 }}>{claim.claim}</div></div>)}</div> : <EmptyModule status={statuses?.compliance} />}
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.15fr 0.85fr", gap: 16 }}>
        <div style={cardStyle}>
          <ModuleHeading module="news" status={statuses?.news} />
          {intel?.news?.length ? <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{intel.news.slice(0, 6).map((item) => <a key={item.url} href={item.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "block", borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: 10 }}><div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 3 }}><span style={{ color: "#50e3c2", fontSize: 9, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase" }}>{item.label}</span>{item.publishedAt && <span style={{ color: "rgba(255,255,255,0.28)", fontSize: 10 }}>{new Date(item.publishedAt).toLocaleDateString()}</span>}</div><div style={{ color: "rgba(255,255,255,0.78)", fontSize: 12, lineHeight: 1.45 }}>{item.headline}<EvidenceLinks items={item.evidence} /></div></a>)}</div> : <EmptyModule status={statuses?.news} />}
        </div>

        <div style={cardStyle}>
          <ModuleHeading module="integrations" status={statuses?.integrations} />
          {intel?.integrations?.length ? <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>{intel.integrations.slice(0, 30).map((integration) => <a key={integration.name} href={integration.url || "#"} target="_blank" rel="noreferrer" title={integration.evidence?.[0]?.excerpt || integration.name} style={{ textDecoration: "none", fontSize: 11, border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.65)", background: "rgba(255,255,255,0.04)", borderRadius: 7, padding: "6px 8px" }}>{integration.name}</a>)}</div> : <EmptyModule status={statuses?.integrations} />}
        </div>
      </div>

      <div style={cardStyle}>
        <ModuleHeading module="productPricing" status={statuses?.productPricing} />
        {intel?.productPricing && (intel.productPricing.productClaims.length || intel.productPricing.pricingStatement) ? <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 20 }}><div><div style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 7 }}>Published product claims</div><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{intel.productPricing.productClaims.slice(0, 6).map((claim) => <div key={claim} style={{ color: "rgba(255,255,255,0.68)", fontSize: 12, lineHeight: 1.45 }}>• {claim}<EvidenceLinks items={intel.productPricing!.claimEvidence?.[claim]} /></div>)}</div></div><div><div style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 7 }}>Pricing / conversion</div><div style={{ color: "rgba(255,255,255,0.68)", fontSize: 12, lineHeight: 1.5 }}>{intel.productPricing.pricingStatement || "No public pricing statement found."}<EvidenceLinks items={intel.productPricing.pricingEvidence || intel.productPricing.evidence} /></div>{intel.productPricing.primaryCta && <div style={{ color: "#50e3c2", fontSize: 11, marginTop: 10 }}>Primary CTA: {intel.productPricing.primaryCta}</div>}</div></div> : <EmptyModule status={statuses?.productPricing} />}
      </div>
    </section>
  );
}
