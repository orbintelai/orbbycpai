...
function BlockedModule({ status }: { status?: ModuleStatus }) {
  return <p style={{ margin: 0, color: "rgba(255,255,255,0.48)", fontSize: 12, lineHeight: 1.6 }}>{status?.reason || "The relevant first-party source restricts automated access."}</p>;
}
...
function CoverageStrip({ statuses }: { statuses: CompanyIntelligence["moduleStatuses"] }) {
  const unavailable = MODULES.filter((module) => statuses[module]?.status === "not_published");
  ...
  return (
    <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", padding: "13px 2px 0", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      {unavailable.map((module) => <span key={module} title={statuses[module]?.reason || "This company has not published this information on its first-party website."} style={{ color: "rgba(255,255,255,0.54)", fontSize: 11, cursor: "help" }}>{LABELS[module]}</span>)}
    </div>
  );
}
...
export function CompanyIntelligencePanel({ profile, generationId }: { profile: { companyIntelligence?: CompanyIntelligence }; generationId: string }) {
  ...
  const visible = (module: IntelligenceModule, hasPublishedData: boolean) => hasPublishedData || statuses[module]?.status === "blocked";
